...
HOOKS_DIR="$CLAUDE_DIR/semantic-search-hooks"
...
copy_hook_files() {
    ...
    # Copy main scripts
    cp "$script_dir/query-router.sh" "$HOOKS_DIR/"
    cp "$script_dir/context-preparer.sh" "$HOOKS_DIR/"
    cp "$script_dir/result-processor.sh" "$HOOKS_DIR/" 2>/dev/null || true
    cp "$script_dir/test-system.sh" "$HOOKS_DIR/"
    
    # Copy library files
    cp -r "$script_dir/lib/"* "$HOOKS_DIR/lib/"
    
    # Copy documentation
    cp -r "$script_dir/docs/"* "$HOOKS_DIR/docs/" 2>/dev/null || true
    cp "$script_dir/README.md" "$HOOKS_DIR/" 2>/dev/null || true
...
configure_claude_hooks() {
    ...
    local hooks_config=$(jq --arg hooks_dir "$HOOKS_DIR" '.hooks = {
        "UserPromptSubmit": [
            {
                "command": ($hooks_dir + "/query-router.sh"),
        ...
        "PreToolUse": [
            {
                "command": ($hooks_dir + "/context-preparer.sh"),
